    ...
    <div v-if='isForgetpas' class="user">
		...
		<span class="install">
			...
			<el-dropdown-menu slot="dropdown">
				<span @click="isshowinstall = true" style="cursor: pointer;">
					<el-dropdown-item>修改密码</el-dropdown-item>
				</span>
				<span @click="showPersonalinformation" v-if="isInformation == '1'" style="cursor: pointer;">
	...
	<div id="personal">
		...
		>
			<el-form :model="information" ref="information" label-width="20%" class="demo-ruleForm">
				<div class="formTable">
					<div class="block">
						<el-form-item label="姓名" prop="realname" :rules="[{ required: true, message: ' '}]">
							<el-input v-model="information.realname" maxlength="50" ></el-input>
						</el-form-item>
					</div>
					<div class="block">
						<el-form-item label="手机号" prop="phonenum" :rules="[{ required: true, message: ' '}]">
							<el-input v-model="information.phonenum" maxlength="50"  disabled></el-input>
						</el-form-item>
					</div>
					<div class="block upLoad">
						<el-form-item label="头像" prop="img" :rules="[{ required: true, message: ' '}]">
							<div class="Inf-img">
								<div class="imgCont">
					...
					<!-- <div class="block" id="regionChangemodule">
						<el-form-item label="地区" prop="region_full_name" :rules="[{ required: true, message: ' '}]" id='areaSelectpopover2'>
							<el-input
					...
					<!-- 部门 -->
					<div class="block"  id='areaSelectpopover2'>
						<el-form-item label="工作单位" prop="departName">
							...
							<span class="close"  v-if="isDepartdata" @click="closeDepartdata">
								<img src="../assets/close.png" alt="close">
							</span>
						</el-form-item>
					</div>
					...
					<div class="block">
						<el-form-item label="管理员" prop="is_admin" :rules="[{ required: true, message: ' '}]">
					...
					<div class="block">
						<el-form-item label="添加原因" prop="add_reason" :rules="[{ required: true, message: ' '}]">
...
import {getCaption} from '../page/untils/heightAuto'
  ...
  methods: {
		...
		getInitdata(){
			...
			getPersonalinfo(objData).then(res => {
				if (res.status === 200 && res.data.result == "ok") {
					...
					this.information.id = res.data.data.id
					// realname name
		...
		Parameters(){
			...
			let objData = {
				...
				// 新增企业部门id
				"enterprise_depart_id" : info.enterprise_depart.id,
...
#areaSelectpopover2 .close img{
    display: block;
    width: 100%;
    height: 100%;
}
#areaSelectpopover2 .makeSuer{
    position: absolute;
    top: 237px;
    right: 10px;
    width: 60px;
...
#areaSelectpopover2 .el-form-item__label{
    height: 36px;
    margin: 2px 0;
    line-height: 36px;
    border: 1px solid #3b4872;
}
#areaSelectpopover2 .el-input__inner{
    height: 36px;
    margin: 2px 0;
    line-height: 36px;
    border: 1px solid #3b4872;
