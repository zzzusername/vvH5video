<template>
  ...
  <div class="mRight" id="MonitorPage">
          ...
          <div class="formTable">
            ...
            <div class="block">
              ...
              >
                ...
                <div class="block">
              <el-form-item
                label="WebRTCJanusUrl:"
                prop="webrtc_janus_url"
              >
                <el-input v-model="streamServeradd.webrtc_janus_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
                <div class="block">
              <el-form-item
                ...
                prop="WebRTCP2pUrl"
              >
                <el-input v-model="streamServeradd.webrtc_p2p_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
                <div class="block">
              <el-form-item
                ...
                prop="WebRTCTurnUrl"
              >
                <el-input v-model="streamServeradd.webrtc_turn_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
          </div>
          <div class="userBtn2">
    ...
    <div id="MonitorPageEditModel">
      ...
      >
        <el-form :model="streamServeredit" ref="streamServeredit" label-width="30%" class="demo-ruleForm">
          <div class="formTable">
              <div class="block">
              <el-form-item
                label="Vmserver名称："
            ...
            <div class="block">
              ...
              <el-form-item
                ...
                prop="webrtc_janus_url"
              >
                <el-input v-model="streamServeredit.webrtc_janus_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
               <div class="block">
             <el-form-item
                ...
                prop="WebRTCP2pUrl"
              >
                <el-input v-model="streamServeredit.webrtc_p2p_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
               <div class="block">
                <el-form-item
                ...
                prop="WebRTCTurnUrl"
              >
                <el-input v-model="streamServeredit.webrtc_turn_url" maxlength="50"></el-input>
              </el-form-item>
            </div>
                ...
                <div class="cont">
                  <el-table  :data="streamServeretableData" style="width: 100%">
                    <el-table-column prop="" label="流媒体名称">
                      <template slot-scope="scope">
                          <el-input id='streamTableinp' class='tableInp' size="mini" v-model="scope.row.server_name" @input="isshowchange" placeholder="请输入流媒体名称"></el-input>
                       </template>
                    </el-table-column>
                    <el-table-column prop="org_code" label="地区编码"></el-table-column>
                    <el-table-column prop="" label="优先级">
                      <template slot-scope="scope">
                          <el-input id='streamTableinp' class='tableInp' @input="isshowchange"  size="mini" v-model="scope.row.priority" placeholder="请输入流媒体名称"></el-input>
                       </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="" label="操作">
                      <template slot-scope="scope">
                        <!--  v-if='whetherDatachanges'  -->
                        <span v-show="isshowbutton" class="btnSpan" @click="streamTablelistSavedata(scope.row)">保存</span>
...
// stream_server_get_stream_server  获取指定ID的流媒体拉流服务器信息
import {
  ...
  stream_server_get_stream_server,
...
export default {
  ...
  // watch: {
  //     streamServeretableData(val, oldVal) {
  ...
  methods: {
   ...
   this.isshowbutton=true
    ...
    getInitstreamServerloadstream(node,resolve,resData){
      ...
      stream_server_load_stream_servers(objData).then(res => {
        ...
        if (res.status === 200 && res.data.result == "ok") {
         ...
         if (res.data.result == "error") {
            _this.$message({
              message: "从vmserver加载流媒体信息失败，请检查vmserver服务器是否正常",
              type: 'warning'
            });
                resolve([])
            // console.log(res);
    ...
    streamTablelistSavedata(scope){
      // 对比数据是否有修改
      console.log(scope);
      let objDatascope = scope
      let name = scope.server_name;
      let exclusive = scope.exclusive;
      let priority = scope.priority;

      // 查询 流媒体信息 对比  stream_server_get_stream_server
      let objData = {
        "id": scope.id
      }
      stream_server_get_stream_server(objData).then(res => {
          if (res.status === 200 && res.data.result == "ok") {
            // 获取 原有数据
            // server_name exclusive  priority
            let sourceName = res.data.data.server_name;
            let sourceExclusive = res.data.data.exclusive;
            let sourcePriority = res.data.data.priority;

            if(name == sourceName && exclusive == sourceExclusive && priority == sourcePriority){
                //console.log('数据无修改');
                this.$message.error('请检查修改数据');
            }else{
                this.modifyStreamingmedia(objDatascope)
            }
          }
      });	
    },
    // 修改流媒体信息 stream_server_modify_stream_server
    modifyStreamingmedia(data){
        //console.log('修改接口')
        let formData = data;
        let objData = {
        ...
        stream_server_modify_stream_server(objData).then(res => {
          if (res.status === 200 && res.data.result == "ok") {
            //console.log(res);
...
#MonitorPageEditStream .el-input--mini .el-input__inner{
 text-align: center;
}
