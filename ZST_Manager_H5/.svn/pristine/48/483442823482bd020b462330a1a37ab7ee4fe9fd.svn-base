					...
					<el-tab-pane label="流媒体服务器" name="second">
						<div class="vmServercont">
							...
							<div class="serverRight">
								<span>已选数据</span>
								<ul>
									<li class="serverRightli" v-for='item in serverMediacheckData' :key="item.index">
										{{item.server_name}} ( {{item.vmserver.name}} )
											<img src="../../assets/close.png" alt="删除" @click="deleteserver(item)">
									</li>
...
deleteserver(a,b){
	this.allmonitoreditId=[];
					var _this=this;
				var otherOrgIds=this.serverMediacheckData
				for (var i = 0; i < otherOrgIds.length; i++) {
					console.log(otherOrgIds[i]);
					console.log(a)

				if(otherOrgIds[i].id==a.id){
					otherOrgIds.splice(i,1)
					
				}
				// more statements
				}
				console.log(otherOrgIds)

					otherOrgIds.forEach(function(item, i) {
											_this.allmonitoreditId.push(
												{id:item.id}
											) });
				console.log(_this.allmonitoreditId)	
				 this.$refs.serverTree.setCheckedNodes(_this.allmonitoreditId);					
	},
    ...
    if(!isNaN(val)){
      ...
      this.$message.error('请输入数字手机号');
		...
		getInitvmServerdata(){
	  	...
	  	// 初始化企业可分配功能
	  	getInitenterpriceFun(type,data){
			  	console.log(data)
			  if(type){
				  var enterprise_id={
					...
					console.log("编辑")
					getenterprise_all_functions(enterprise_id).then(res => {
							if (res.status === 200 && res.data.result == "ok") {
									// 需要过滤下数据
									let resData = res.data.data;
									/* 后期可能会删除此代码 */
									...
									this.allocated_enterprise_functions = resData;
										console.log(this.allocated_enterprise_functions)
							}else{
								// 错误提示信息
								this.$message.error(res.data.error_description);
							}
		...
		serverCheckchange(data){
			...
			console.log(this.serverMediacheckData)
			...
			if(!data.parent){
				if(checked){
					...
					_this.serverMediacheckData.push(data)
						console.log(_this.serverMediacheckData)
					var listname=[];
					
					listname=_this.removing_server(_this.serverMediacheckData);
	...
	removing_server(idlist){
  var obj={};

  for(var i=0;i<idlist.length;i++){
	var cur=idlist[i].id; // 100 100 900.....
	console.log(cur)
    if(obj[cur]===cur){
	...
	return  idlist
		...
		AEenterpriseClick(type,data) {
			...
			if(type){
				...
				// 编辑
					this.getInitenterpriceFun(true,data);
...
#tabChagecontEnterprise .vmServercont .serverRight li{
	height: 26px;
    line-height: 26px;
    vertical-align: middle;

}
#tabChagecontEnterprise .vmServercont .serverRight li img{
vertical-align: middle;
