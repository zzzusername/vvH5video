						...
						>
							...
							<el-table-column prop="" label="预览图片">
                                <template slot-scope="scope">
									<!-- window.ServerUrl + scope.row.icon_url -->
                                    <img class="tableImg" :src="imageProcessing(scope.row.icon_url)" alt="">
								</template>
                            </el-table-column>
                            <el-table-column prop="description" label="功能描述"></el-table-column>
							<!-- <el-table-column prop="" label="禁用／启用">
								<template slot-scope="scope">
									 <div id="changeState">
										<el-switch
        ...
        <div id="commonPop">
            <el-dialog  :title="popTitle"
                ...
                <el-form  ref="edit" label-width="35%" class="demo-ruleForm">
                    <div class="formTable">
						<div class="block">
							<el-form-item label="模块名称：" :rules="[{ required: true, message: ' '}]">
						...
						<div class="block upLoad"  id="personal">
							...
							<span id="upImgspan">（168x168像素，png，jpg格式）</span>
								<div class="Inf-img">
									...
									<div class="upCont" v-if="!defaultDatamodification">
										<input
						...
						<div class="block" v-if='!defaultDatamodification'>
							<el-form-item label="跳转链接:(建议以http/https开头)" :rules="[{ required: true, message: ' '}]">
								<el-input v-model="addForm.description" maxlength="50"></el-input>
							</el-form-item>
                        </div>
						<div class="block" v-if='!defaultDatamodification'>
                            <el-form-item label="状态：" :rules="[{ required: true, message: ' '}]">
                    ...
                    <div class="userBtn">
                        <el-form-item>
                            <el-button type="primary" @click="saveNewdata" :loading="saveLoading">保存</el-button>
	...
	export default {
        data() {
			return {
				...
				popTitle : '',
				// 默认数据修改
				defaultDatamodification : false,
				editThevideoName : false,
				// 编辑缓存数据
				scopeData : {},
				/* 分页相关 */
				page_size : 8,			//  请求多少条目
		...
		methods:{
			...
			addDatalist(type,scope){
				...
				if(type){
					// 新增
          this.defaultDatamodification = false;
					...
					// 判断是否为默认数据
					if(scope.sys_default){
						this.defaultDatamodification = true;
					}else{
						this.defaultDatamodification = false;
					}
					this.popTitile = '编辑'
			...
			saveNewdata(){
				...
				if(!this.defaultDatamodification){
					// 验证 http 字端
					var reg = /(http|https):\/\/([\w.]+\/?)\S*/;
					if (!reg.test(objData.description)) {
						this.$message.error('请输入http/https开头的网址');
				...
				if(this.saveBtnstate){
					...
					// 判断 是否编辑  分享圈模块 related_fun: "funShareCircle"
					if(this.scopeData.related_fun == 'funShareCircle'){
						console.log('编辑分享圈名字');
						this.editThevideoName = true;
					}
					//编辑
					...
					appFunmodulesModify(objData).then(res => {
						if (res.status === 200 && res.data.result == "ok") {
							...
							// 编辑分享圈名字 成功
							if(this.editThevideoName){
								//this.$store.commit('setVideoname',this.addForm.name);
								console.log('vuex 视频名称成功');

								console.log($("#videoClass .textName"));
								console.log(objData.name);
								$("#videoClass .textName").html(objData.name);
								console.log($("#videoClass .textName"));
							}
