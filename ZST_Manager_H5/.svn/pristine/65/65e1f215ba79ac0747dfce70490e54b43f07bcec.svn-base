			...
			<div class="formTable">
				...
				<div class="block block-line">
					<el-form-item label="用户类型：" :rules="[{ required: true, message: ' '}]" >
						<el-select v-model="prisegRoupsvalue"  placeholder="--请选择--" :disabled='this.userTypedisabled'>
				...
				<div class="block" id="streamingMedia">
					<el-form-item label="指定流媒体服务器：" label-width="22%">
						<el-select v-model="streamingMediavalue"
								...
								:value="item.id"
								>{{item.server_name}}{{item.vmserver_name}}</el-option>
						</el-select>
					</el-form-item>
				</div>
						...
						<el-tab-pane label="监控资源分配：" name="second"  class="tabSelectpane">
							<div class="tabCont">
								<div class="hd monitorResourceshd">
									<span>监控平台地址：</span>
									<el-select v-model="monitorTheaddressValue"  placeholder="--请选择--" class="monitorResourcesSelect" @change='monitorAddresschange' @visible-change="monitorAddval(monitorTheaddressValue)">
...
export default {
	props : {
  	...
  	data: function() {
		return {
			//判断是不是第一次从 “-请选择-”到xxx唐古拉的时候 不需要提示“是否要切换监控地址”
			is_monitorAdd_change:true,
			// 来源父组件编辑数据
			...
			/* 终端数据 */
			//  获取 当前 分配的  终端服务器信息
			termainlServerinfoIdformEdit : '',
			// 获取可分配的终端通讯录同步服务器信息 ID
	...
	methods:{
		...
		distinguishPagesources: function (data) {
			...
			if(path == 'USERLIST'){
				...
				// userTypedisabled
				if(parentDetailId == localStorage.userId){
		...
		/* 用户审核页面初始化信息 */
		//  获取可分配的终端通讯录地区根目录  用户审核
		getInituserCheckterminalRegion(){
			// get_root_terminal_region  根据 用户和地区 区分
			// 判断当前区域是否为全国
...
2,4 为企业管理员  SUPER_NATIONWIDE 为全国节点 SUPER_NOO_NATIONWIDE 为非全国节点 */
		...
		getInituserCheckterminalServer(){
			...
			get_terminal_server(objData).then(res => {
				if (res.status === 200 && res.data.result == "ok") {
					...
					// 初始化 终端 根节点数据  有可能会根据  平台id 加载根节点
		...
		userListgetInitdata(id){
			...
			getAssignableprivileges(objData).then(res => {
				if (res.status === 200 && res.data.result == "ok") {
					...
					// 监控地址赋值
					this.monitorTheaddressValue = rdData.monitor_server_id ? rdData.monitor_server_id  : '';
					this.monitorTheaddressValue = rdData.monitor_server_id ? rdData.monitor_server_id  : '';
					...
					this.termainlServerinfoIdformEdit = rdData.terminal_server_id
		...
		getuserTerminalserver(){
			userListgetTerminalserver({  "enterprise_id": localStorage.EnterpriseId}).then(res => {
				if (res.status === 200 && res.data.result == "ok") {
					...
					// 选择平台不一致
					if(this.termainlServerinfoIdformEdit != this.termainlServerinfoId ){
						console.log("平台不一致")
						// 清空 勾选信息
						this.terminalDefaultcheckedCopy = [];
						// 根据 终端同步的服务器信息 ID 获取 终端根节点数据
						this.getTerminalrootNodedirectory();
					}else{
						// 根据 终端同步的服务器信息 ID 获取 终端根节点数据
						this.getTerminalrootNodedirectory();
		...
		monitorAddval(val){
			...
			if(val===''){
				this.is_monitorAdd_change=true;
			}else{
				this.is_monitorAdd_change=false;
			}
		},
		// 监控地址 change
		monitorAddresschange(value){
			...
			if(this.is_monitorAdd_change){
				...
				this.$confirm('切换监控平台地址会清空已勾选的监控资源，确定要切换?', '提示', {
...
2,4 为企业管理员  SUPER_NATIONWIDE 为全国节点 SUPER_NOO_NATIONWIDE 为非全国节点 */
			if(differentiateSite == 'REGION_NATIONWIDE' || differentiateSite == 'SUPER_NATIONWIDE'){
				...
				listSuborganizations(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						if(res.data.error_description = 'No route to host (Host unreachable)'){
		...
		// 监控根数据修改
		monitorInterfacedataProcessing(data){
			// 赋值
			let array = data;
			let sortingArray = []
			// 多个平台 遍历内容去最后一个节点
...
2,4 为企业管理员  SUPER_NATIONWIDE 为全国节点 SUPER_NOO_NATIONWIDE 为非全国节点 */
		...
		getTermianloadNode(node,resolve){
			...
			if(node.data.unit_id){
				...
				if(node.data.unit_id != '-1'){
					let objData = {
						"terminal_server_id": _this.termainlServerinfoId,
		...
		termianCheckchangdata(data, type, childState){
			...
			if(type){
				...
				if(this.pageSourcedata == 'USERLIST'){
					...
					this.terminalDefaultchecked.push(_data)
				}
			}else{
				//console.log("删除");
				// 减 遍历删除
		...
		childSavedata(){
			...
			if(this.pageSourcedata == 'USERLIST'){
				...
				let objData = this.userCheckparameters();
		...
		userListparameters(){
			...
			if(this.monitorDefaultchecked != []){
				...
				this.monitorDefaultchecked.map(function(item,index){
					let obj = {};
					obj.region_name = item.region_name ?  item.region_name  :  item.name
					obj.region_code = item.region_code ? item.region_code :  item.area_code
