		...
		methods:{
				...
				listParentmonitorResourceorganizationPaths(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						if(this.monitorDefaultchecked.length != 0){
							...
							this.monitorDatacheckedDefault(disArray,'monitorTree');
						}
					}else{
						// 错误提示信息
						this.$message.error(res.data.error_description);
			...
			monitorCheckchangdata(data, type, childState){
				...
				if(type){
					...
					this.monitorDefaultchecked = clearChildarray;
					// 添加当前选中的数据  合并 去重
			...
			monitorResourcetree(node,resolve){
				//console.log('监控 tree')
				// 缓存node 信息 首次调用 
				...
				if (node.level === 0) {
					return resolve(this.monitorResourceallocationdata);
				}
			...
			terminalAddressbookTree(node,resolve){
				console.log('终端 tree')
				// 缓存node 信息 首次调用 
				let first = true;
				if(first){
			...
			termianCheckchangdata(data, type, childState){
				...
				if(type){
					...
					this.terminalDefaultchecked = clearChildarray;
					// 添加当前选中的数据 去重
			...
			getDetailedcontentAndoperations(objData){
					...
					getSubenterpriseInfo(objData).then(res => {
						if (res.status === 200 && res.data.result == "ok") {
							...
							this.monitorDefaultcheckedCopy = rD.monitor_resource_organizations;
							// 流媒体赋值
							this.enterpriseStreamserversValue = rD.stream_servers[0].server_code;	

							console.log(this.enterpriseStreamserversValue)
							...
							// 监控初始化渲染数据
							let monitorArray = this.monitorResourceallocationdata;
							// 获取 监控 源数据
							let defaultCheckedmonitorArray = res.data.data.monitor_resource_organizations;
							...
							this.terminalDefaultcheckedCopy = rD.terminal_regions;
							...
							// 获取 终端 根节点 初始化数据
							let defaultArray = this.terminalAddressbookTreedata;
							// 获取 终端 未处理的数据
							let defaultCheckedarray = rD.terminal_regions;
							// 获取 终端 半勾选数组
