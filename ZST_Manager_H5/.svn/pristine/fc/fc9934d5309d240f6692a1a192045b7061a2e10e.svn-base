                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <a href="javascript:;" class="ml5"  v-if="scope.row.is_new_public == true">
                      <span  @click="appedit(scope.row)">编辑</span>
                    ...
                    <a href="javascript:;" class="ml5" @click="butclick(scope.row)"  v-if="scope.row.is_new_public == true">
                      ...
                      <button :disabled=isbutton  class="butclick" v-if="scope.row.enabled == true">禁用</button>
                      <button :disabled=isbutton  class="butclick" v-else>启用</button>
          ...
          <div class="formTable">
            ...
            <div class="block">
              <el-form-item label="版本名称" :rules="[{ required: true, message: ' '}]" prop="version_name" >
                <el-input v-model="add.version_name" maxlength="50"  :disabled=isdisabled_add  placeholder="--请输入版本名称--"></el-input>
              </el-form-item>
              <!-- <div class="infoMsg">
             ...
             <div class="block">
              <el-form-item label="版本号"  :rules="[{ required: true, message: ' '}]" prop="version_code" >
                <el-input v-model="add.version_code" maxlength="50" :disabled=isdisabled_add  placeholder="--请输入版本号--"></el-input>
              </el-form-item>
              <!-- <div class="infoMsg">
            ...
            <div class="block">
              <el-form-item label="操作系统" :rules="[{ required: true, message: ' '}]" prop="name">
                <el-select v-model="add.operating_system" @change="geoperating_system" placeholder="--请选择--">
                  <el-option v-for="item in options_system"     :disabled=isdisabled_add   :key="item.index" :label="item.value"
       ...
       Managementlist_app_upgrade_infos(par).then(function (res) {
          ...
          if (res.status === 200 && res.data.result == "ok") {
            ...
            for (var i = 0; i < response.length; i++) {
              if(response[i].force_upgrade_time!=undefined ){
      ...
      getUploadVideo() {
        ...
        if (this.filedata == "") {
          ...
          return false
            ...
            var reg =/\s/;
           if (reg.test(this.filedata.name)) {
          this.$message({
            message: '文件名不允许有空格',
            type: 'warning',
...
//              });
      ...
      appedit(row) {
        ...
        if( _this.edit.force_upgrade_time){
...
//            console.log(res);
      ...
      butclick(al){
        ...
        this.$confirm('是否执行'+isenabled+'操作?', '消息', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
...
<style scoped>
  ...
  #userAddModel #files {
	...
	#userAddModel .block .el-radio-group{
		margin-left: 10%
	}
  #userAddModel .formbox .el-radio-group{
		margin-left: 5%
	}
  .sccess {
    color: #fff;
