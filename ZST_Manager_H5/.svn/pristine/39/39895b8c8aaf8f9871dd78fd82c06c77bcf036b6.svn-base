<template>
	<div id="usercheck" class="mRight">
				...
				<div class="elTable">
					<div class="scrollbox">
						...
						>
							...
							<el-table-column prop="enterprise_depart.name" label="工作单位"></el-table-column>
							<el-table-column prop="" label="权限分组">
								<template slot-scope="scope">
									<span v-if="scope.row.enterprise_group">{{scope.row.enterprise_group.name}}</span>
							...
							<el-table-column prop="" label="角色">
								<template slot-scope="scope">
									<!-- 企业管理员 地区管理员 -->
									<!-- <span v-if="scope.row.is_casual_admin ? text='临时管理员' : scope.row.is_admin ? scope.row.is_district_admin ? text='地区管理员' : text='企业管理员' : text='普通用户'  ">{{text}}</span> -->
									<span v-if="scope.row.is_admin ? (scope.row.is_super_admin || scope.row.is_casual_admin ? text='企业超级管理员' : (scope.row.is_district_admin ? text='企业地区管理员' : '未知')) : text='普通用户'">{{text}}</span>
							...
							<el-table-column prop="" label="操作" width="170px">
								<template slot-scope="scope">
									<span v-if='!scope.row.is_casual_admin' class="spanBtn" @click="assignPermissionsclick(scope.row)">分配权限</span>
									<span class="spanBtn" @click="getUserdetail(scope.row)">详情</span>
									<span v-if='scope.row.id != enterpriseUserid' class="spanBtn" @click="deleteUserdata(scope.row)">删除</span>
			...
			<div class="block block-inline">
				...
				<!-- <li>
		 		...
		 		<li>
					...
					<span class="liR">{{userDeatiledata.region_name ? userDeatiledata.region_name  : '暂无数据'}}</span>
				</li>
			<!-- 	<li>
            ...
            <el-dialog  title="新增" 
                ...
                <el-form  ref="edit" label-width="30%" class="demo-ruleForm">
                    <div class="formTable">
						...
						<div class="block block-line">
							<el-form-item label="确认密码：" :rules="[{ required: true, message: ' '}]">
						...
						<div class="block" v-if='this.cartIdcontrol != "HIDDEN"'>
							<el-form-item label="身份证号码：" label-width="15%">
								<el-input v-model="addForm.id_card" maxlength="50"></el-input>
							</el-form-item>
                        </div>
						<div class="block">
                            <el-form-item label="用户类型："  label-width="15%">
                                <div class="checkboxBg">
                                    <el-radio v-model="addForm.userTypes" label="ENTERPRISE_ADMIN">普通用户</el-radio>
  									<el-radio v-model="addForm.userTypes" label="Enterprise_AREA_ADMIN">地区管理员</el-radio>
  									<el-radio v-model="addForm.userTypes" label="ENTERPRISE_SUPER_ADMIN">超级管理员</el-radio>
                                </div>
                            </el-form-item>
                        </div>
						<div class="block" id="areaSelectpopover">
							<el-form-item label="地区：" label-width="15%" :rules="[{ required: true, message: ' '}]">
								<el-input 
						...
						<!-- 部门 depart -->
						<div class="block" id="areaSelectpopover" v-if='this.departControl != "HIDDEN"'>
							<el-form-item label="工作单位：" prop="depart" label-width="15%">
								<el-input 
						...
						<div class="block">
                            <el-form-item label="职务："  label-width="15%" v-if='this.jobTitleControl != "HIDDEN"'>
						...
						<!-- 
                        <div class="block">
                            <el-form-item label="权限分组：" label-width="15%" :rules="[{ required: true, message: ' '}]">
        ...
        <!-- 权限分配弹窗 -->
        <div id="commonPop">
            <tabTree  ref="tabTreemark" :pageSource='pageSource' :assignPopchild='assignPopchild' @assignPopclose='closePop'></tabTree>
        </div>
	</div>
</template>
<script>
	...
	// listRegistrationsettings  列出企业用户注册信息字段具体的约束值
	import {
	...
	export default {
        data() {
			return {
				enterpriseUserid : localStorage.getItem('userId'),
				...
				addForm : {
					...
					sex : 'BOY',									// 性别
					id_card : '',									// 身份证信息
					userTypes : 'ENTERPRISE_ADMIN',						// 用户类型
				},
				// 权限数据列表
				prisegRoupsdata : [],
				prisegRoupsvalue : '',
		...
		methods:{
			...
			// 初始化 企业用户 约束值 listRegistrationsettings
			gitInitconstraints(){
				let _this = this;
				let objData = {
					"enterprise_id": localStorage.EnterpriseId,
				}
				listRegistrationsettings(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						resData.map((item,index) => {
							jurisdictionObj[item.fieldName] = item.fieldConstraintValue
						})
						this.sexControl  = jurisdictionObj.sex
						this.regionControl  = jurisdictionObj.region
						this.departControl  = jurisdictionObj.depart
						this.jobTitleControl  = jurisdictionObj.jobTitle
						this.cartIdcontrol  = jurisdictionObj.idCard
					}
				});
			},
			// 初始化获取所有用户
			...
			getInituserRolesdata(){
				...
				userListroles(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
			...
			getInituserStatus(){
				// userListstatuses
				let objData = {}
				userListstatuses(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
					...
					if (res.status === 200 && res.data.result == "ok") {
						...
						if(res.data.data.is_district_admin){
							...
							this.InitRegionfulldata = {
								name : res.data.data.region_name,
								id : res.data.data.region_code
							}
						}else{
							console.log('超级管理员')
							this.InitRegionfulldata = {
								name : res.data.data.enterprise.region_name,
								id : res.data.data.enterprise.region_code
							}
						}	
					}
				});
			...
			saveNewdata(){
				let objData = {
					...
					"user_type" : this.addForm.userTypes,
					"id_card" : this.addForm.id_card,
					"sex" : this.addForm.sex,
				}
				// 性别
				/* if(this.sexControl = 'HIDDEN'){
					// 不现实为空
					objData.sex = 'BOY';
				}else{
					// 显示
					objData.sex = this.addForm.sex;
				}; */
				
				// 地区
				// 必填 REQUIRED
				/* if(this.regionControl == 'REQUIRED'){
					if(this.information.region_code == ''){
						this.$message.error('请选择地区');
						return false;
					}
				}else if(this.regionControl == 'OPTIONAL'){
					objData.region_code = this.information.region_code;
					objData.region_name = this.information.region_name;
				}else if(this.regionControl == 'HIDDEN'){
					objData.region_code = ''
					objData.region_code = ''
				}
				 */
				...
				// 部门 this.departControl
				if(this.departControl == 'REQUIRED'){
					if(this.enterPrisepartsvalue == ''){
						this.$message.error('请选择工作单位');
						return false;
					}
				}else if(this.departControl == 'OPTIONAL'){
					objData.depart_id = this.enterPrisepartsvalue;
				}else if(this.departControl == 'HIDDEN'){
					objData.depart_id = ''
				}

				// 职务 job_title_id
				if(this.jobTitleControl == 'REQUIRED'){
					if(this.addForm.job_title_id == ''){
						this.$message.error('请选择职务');
						return false;
					}
				}else if(this.jobTitleControl == 'OPTIONAL'){
					objData.job_title_id = this.addForm.job_title_id;
				}else if(this.jobTitleControl == 'HIDDEN'){
					objData.job_title_id = ''
				}

				// 身份证号码验证
				if(this.cartIdcontrol == 'REQUIRED'){
					if(this.addForm.id_card == ''){
						this.$message.error('请输入身份证号码');
						return false;
					}
				}else if(this.cartIdcontrol == 'OPTIONAL'){
					objData.id_card = this.addForm.id_card;
				}else if(this.cartIdcontrol == 'HIDDEN'){
					objData.id_card = ''
				}
				// 验证密码
			...
			getUserdetail(scope){
				...
				getUserlistDetail(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						this.enterprise_groups = res.data.data.enterprise_groups ? res.data.data.enterprise_groups[0] : '暂无数据';
			...
			getUploadVideo() {
				...
				if (this.filedata == "") {
					...
					// 验证 fileName
					let strings = this.filedata.name;

					if(isWhitespace(strings)){
						this.$message({
							message: '文件名字不允许带有空格 请重新上传',
							type: 'warning',
						});
						return false;
					}
            ...
            /* 分配权限 */
            assignPermissionsclick(scope){
                // 访问分配权限详情接口  获取数据 传入子组件， 来源页面  usercheck/userlist
				console.log('分配权限');
				let parentData = {		
					path : 'USERLIST',
					id : scope.id,
				...
				// 告知子组件页面来源 加载对应数据
				this.$refs.tabTreemark.distinguishPagesources(parentData)
				// 展示弹窗
