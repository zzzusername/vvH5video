        ...
        <div id="commonPop">
            <el-dialog  title="编辑内容" 
                ...
                <el-form  ref="edit" label-width="45%" class="demo-ruleForm">
                    <div class="formTable">
						<div class="block" id="shareContent">
							<div class="textarea" id="shareContenttext">
								<el-form-item label="请输入内容">
									<el-input type="textarea" v-model="addForm.content" resize="none" maxlength="140"></el-input>
								</el-form-item>
							</div>
                        </div>
						<!-- 头像上传 shareImglist -->
                    ...
                    <div class="userBtn">
                        <el-form-item>
                            <el-button type="primary" @click="publishShare" :loading="saveLoading">保存</el-button>
                    ...
                    <div class="formTable">
						<div class="block">
							<el-form-item label="内容：">
								<el-input v-model="detail.content" disabled="disabled"></el-input>
							</el-form-item>
							...
							<el-form-item label="可见朋友：" id="shareImglist" v-if='!detail.enterprise_users'>
								<li class="enterpriseLi" v-for="item in detail.enterprise_users" :key="item.index">{{item.name}}</li>
							</el-form-item>
	...
	export default {
        data() {
			return {
				...
				upLoadapi : window.ServerUrl + '/enterprise_admin/api/v1/share_circle_management/share/upload_resource',
				headers : {
		...
		methods:{
				...
				getVisibilitymap({}).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						for(var i in resData) {
							let obj = {}
							if(resData[i]!="仅自己" && resData[i]!="所有用户"){
								obj.name = resData[i];
								obj.type = i;
								jsonArray.push(obj);
							}
			...
			getDetail(scope){
				...
				shareDetail(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						...
						this.detail.enterprise_users = resData.enterprise_users
						this.detailPop = true;
					}else{
						// 错误提示信息
						this.$message.error(res.data.error_description);
			...
			videoPlay(path){
				...
				// 新建标签页面 地址  url 传入
				let url = path
				let routeData = this.$router.resolve({ name: 'Dynamicvideo', query: {  url: url} });
