			...
			<div class="zTable">
				<div class="elTable">
					<div class="scrollbox">
						...
						:data="setloginData"
						>
							<el-table-column prop="fieldDescription" label="注册信息"></el-table-column>
							<!-- <el-table-column prop="fieldConstraintValue" label="状态"></el-table-column> -->
							<el-table-column prop="" label="状态">
								<template slot-scope="scope">
									<el-radio-group v-model="scope.row.fieldConstraintValue" @change="radioChange(scope.row)">
									    <el-radio label="REQUIRED">必填</el-radio>
									    <el-radio label="OPTIONAL">可选</el-radio>
									    <el-radio label="HIDDEN">不显示</el-radio>
									</el-radio-group>
								</template>
							</el-table-column>
						</el-table>
...
<script>
	...
	/* api */
	import {enterpriseUserregistrationSettinglist,
	listRegistrationfieldConstraints,enterpriseUserregistrationSettingmodify} from '../../api/setlogin'
	...
	export default {
		...
		methods:{
			// 初始化列表数据
			getInitlistdata(){
				// enterpriseUserregistrationSettinglist
				let objData = {
					"enterprise_id": localStorage.EnterpriseId,
				}
				enterpriseUserregistrationSettinglist(objData).then(res => {
			...
			radioChange(scope){
				// scope
				// enterpriseUserregistrationSettingmodify
				let objData = {
					"enterprise_id": localStorage.EnterpriseId,
					"field_constraint_value": scope.fieldConstraintValue,
					"field_name": scope.fieldName
				}
				enterpriseUserregistrationSettingmodify(objData).then(res => {
					if (res.status === 200 && res.data.result == "ok") {
						console.log(res);
