<template>
	<div id="usercheck" class="mRight">
			...
			<div class="zTable">
				<div class="elTable">
					<div class="scrollbox">
						...
						:data="usercheckData"
						>
							<el-table-column prop="fixedSort" label="序号" width="40px"></el-table-column>
		...
		<el-dialog  title="申请详情" 
		 ...
		 <!-- 通过弹窗 -->
        <div id="commonPop">
            <tabTree  ref="tabTreemark" :pageSource='pageSource' :assignPopchild='assignPopchild' @assignPopclose='closePop' @refreshList='refreshList'></tabTree>
        </div>
		<!-- 驳回弹窗 -->
		<div id="commonPop">
	...
	export default {
		...
		methods:{
			...
			passClick(scope){
				...
				let parentData = {		
					path : 'USERCHECK',
					id : scope.enterprise_apply_event_id,
			...
			rejectedClick(scope){
				console.log(scope)
				this.enterprise_apply_event_id = scope.enterprise_apply_event_id;
			...
			submittingBy(){
				// 驳回
				let objData = {
					"enterprise_apply_event_id": this.enterprise_apply_event_id,
			...
			// 驳回清空事件
			cancelRejectedpop(){
				this.rejectedPop = false;
				this.enterprise_apply_event_id = '';
