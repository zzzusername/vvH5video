...
<script>
	...
	/* api */
	// getTerminalserver  获取可分配的终端通讯录同步服务器信息
	// getTerminallist  列出指定单位下的终端设备信息
	// listRootterminalRegionpath  列出当前企业分配的终端通讯录地区目录路径
	// listRootterminalRegion  列出当前企业分配的终端通讯录地区目录
	...
	export default {
		...
		methods:{
			...
			getInitterminalServer(){
				...
				// 初始化数据接口
				getTerminalserver(objData).then(res => {
					console.log(res);
					if(res.data.data === {}){
						return false;
					}else{
						this.terminalServerid = res.data.data.id
						this.getInitdata()
					}
				});	
			...
			getInitdata(){
				...
				// 初始化数据接口
				listRootterminalRegion(objData).then(res => {
					// 数据处理
			...
			getMonitordata(){
				let objData = {
					"root_terminal_region_code": this.InitializeData[0].region_code,
					"terminal_server_id": this.terminalServerid,
					"page_number": this.page_total_pages - 1,
  					"page_size": 10,
				}
				if(this.keyName != ''){
					objData.search_text = this.keyName;
				}
				// 搜索终端设备信息
