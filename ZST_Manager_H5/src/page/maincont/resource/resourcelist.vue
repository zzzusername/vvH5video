	...
	<div id="resoure" class="mRight">
		<div class="resoureList">
			<div class="zForm">
				...
				<span class="paddingLeft">监控名称： </span><input v-model="keyName" class="zInput" type="text" placeholder="" />
        <button class="buttonradius" @click="findData">查询</button>
        <button class="buttonradius" @click="clearData">清空</button>
        <div class="monitorResourceshd">
          <span>监控平台地址：</span>
          <el-select v-model="monitorTheaddressValue"  placeholder="--请选择--" class="monitorResourcesSelect" @change='monitorAddresschange($event)'>
...
<script>
	...
	// js
	import {heightAuto,distinguishRole} from '../../untils/heightAuto' //注意路径
	/* api */
	...
	getMonitordetail,listEnterprisemonitorServers,
	} from  '../../api/resoure'

	// 查询企业信息
	import {getEnterprisinfo} from '../../api/commonapi'
	export default {
        data() {
			return {
				// 查询方式 接口切换
		...
		methods:{
			getServers(){
				...
				// 初始化数据接口
				listEnterprisemonitorServers(objData).then(res => {
					console.log(res);
					this.monitorTheaddressData = res.data.data;
					this.monitorTheaddressValue = res.data.data[0].id
					this.monitorTheaddressValueold = res.data.data[0].id
					// 初始化监控地址
					this.getInitdata();
				});
			},
...
2,4 为企业管理员  SUPER_NATIONWIDE 为全国节点 SUPER_NOO_NATIONWIDE 为非全国节点 */
        ...
        this.InitializeData = [];
        // 赋值操作
        this.InitializeData = sortingArray;
        ...
        if (node.level >= 1) {
          this.getMonitorchildrenNode(node,resolve);
			...
			checkThedetails(row){
				...
				getMonitordetail(objData).then(res => {
		...
		watch:{
			screenHeight(val){
				...
				if(!this.timer){
					...
					setTimeout(function(){
						// 打印screenHeight变化的值
						$tree.css("height",that.screenHeight/2);
						that.timer = false
					},400)
				}
			},
...
.monitorResourceshd span{
	float: left;
	height: 36px;
	line-height: 36px;
}
.monitorResourceshd .monitorResourcesSelect{
	float: left;
	width: 400px;
}
.popInput{
