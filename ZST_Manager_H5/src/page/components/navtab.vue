    ...
    <div class="navMenubox">
      <el-row class="tac">
        <el-col :span="24">
          ...
          >
            <el-submenu :index="item.index" v-for="(item,index) in  routingList" :key="index">
              <template v-if="item.children">
                ...
                <el-menu-item-group v-for="(it,index) in item.children" :key="index">
                  <router-link :to="it.url" class="MyOption Myopen Mydefault">
                    <el-menu-item class="MyOptionli" :index="it.url" :id="it.className">
                      <span class="imgico" :class="'img' + it.defaultIcon"></span>
                      <span class="textName">{{it.title}}</span>
                    </el-menu-item>
                  </router-link>
                </el-menu-item-group>
              </template>
...
/* api */
import { appFunmodulesList } from '../api/appmanagement'
...
export default {
  ...
  methods: {
    dynamicRouting() {
      // 获取用户类型判断侧边栏
      let isDistrictadmin = localStorage.isDistrictadmin
      if(isDistrictadmin == 'true'){
        //console.log('该企业为区域管理员');
        this.routingList = this.regionlist.item;
      }else{
        this.routingList = this.defaultlist.item;
      }
    },
    defaultIdex() {
    ...
    getInitvideoName(){
        ...
        let objData = {
          ...
          "page_size": 10
            ...
            //this.$store.state.videoName
            let resData = res.data.data.list;
            resData.map(function(item, index){
                if(item.related_fun == 'funShareCircle'){
                    //_this.$store.commit('setVideoname',item.name);
                    $("#videoClass .textName").text(item.name);
                }
            });
