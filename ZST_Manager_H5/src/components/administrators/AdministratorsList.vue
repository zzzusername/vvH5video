      ...
      <el-dialog :close-on-click-modal="false" title="管理员列表 - 新增" :visible.sync="dialogTableVisibleadd" :before-close="handleClose">
        <el-form :model="admineadd" ref="admineadd" label-width="38%" class="demo-ruleForm">
          <div class="formTable">
            ...
            <div class="block">
              <el-form-item label="手机号" :rules="[{ required: true, message: ' '}]" prop="VersionNum">
                <el-input v-model="admineadd.phonenum" maxlength="11"
                 @change="isRealNum_Add"
...
//          console.log(r)
      ...
      editSubmit() {
        ...
        console.log(par)
        console.log(this.adminedit.phonenum.length)
        ...
        if (this.adminedit.phonenum == ''||this.adminedit.phonenum.length!=11) {
          this.$message({
            message: '请输入11位手机号',
            type: 'warning'
          });
...
//          console.log("6666")
          var reg = new RegExp("^((1[0-9]{1})+\\d{9})$");
          var r = reg.test(this.adminedit.phonenum);
//          console.log(r)
          if (r != true) {
            this.$message({
              message: "手机不符合要求",
...
<style scoped>
 ...
 #adminAddModel .block .el-radio-group {
       margin-left: 25%;
 }
  #adminAddModel #files {
    width: 90px;
